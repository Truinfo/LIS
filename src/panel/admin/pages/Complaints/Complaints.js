...
const PersonalComplaints = ({ complaints, handleEdit, handleDelete }) => (
    <FlatList
        data={complaints}
        renderItem={({ item }) => (
            <ComplaintItem item={item} handleEdit={handleEdit} handleDelete={handleDelete} />
        )}
        ...
        <View style={styles.cardContainer}>
            ...
            <View style={styles.detailsContainer}>
                ...
                <View style={styles.detailRow}>
                    ...
                    <Text style={styles.value}>: {new Date(item.dateAndTime).toLocaleString()}</Text>
                </View>
                <View style={styles.detailRow}>
...
const Complaints = () => {
    ...
    const ImmutableComplaints = useSelector((state) => state.adminComplaints.complaints || []);
        ...
        dispatch(deleteComplaintAsync({ complaintId }))
            .then((result) => {
                if (result.meta.requestStatus === "fulfilled") {
                    ...
                    setSnackbarMessage("Failed to delete the complaint. Please try again.");
                    setSnackbarVisible(true);
                }
            })
            .catch(() => {
    ...
    const handleStatusChange = () => {
        ...
        if (selectedComplaint) {
            dispatch(updateComplaintStatusResolution({ complaintId: selectedComplaint._id, resolution }))
                .then((result) => {
                    if (result.meta.requestStatus === "fulfilled") {
    ...
    const renderScene = SceneMap({
        personal: () => <PersonalComplaints complaints={personalComplaints} handleEdit={handleEdit} handleDelete={handleDelete} />,
